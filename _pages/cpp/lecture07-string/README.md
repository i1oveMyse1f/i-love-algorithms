...
Это происходит, потому что при выводе переменной типа ``char`` С++ считает, что мы хотим вывести символ (англ. _char_). Осталось ответить на главный вопрос:
...
В библиотеке ``<cctype>`` есть [множество функций](https://en.cppreference.com/w/cpp/header/cctype), которые помогут вам работать с символами. Например, функция ``isalpha`` проверяет, является ли символ буквой, а ``isdigit`` - цифрой, ``to_lower`` - переводит в нижний регистр. Вот пример использования:
